...
{!! Form::open( ['id' => 'frmBookingRecall', 'class' => 'form-horizontal','method' => 'post', 'route' => ['ortho.bookings.booking_recall_regist'], 'enctype'=>'multipart/form-data', 'accept-charset'=>'utf-8']) !!}
<section id="page">
      <div class="container">
        <div class="row content-page">
          <h3>各種リスト表示　＞　「リコール月」の新規登録</h3>
          <table class="table table-bordered">
                ...
                <td>
                    <select name="clinic_id" id="clinic_id" class="form-control" style="width: 250px !important;">
                      ...
                      @if(count($clinics) > 0)
                        ...
                        @endforeach
                        @foreach($listClinic as $clinic_id => $clinic)
                        <option value="{{$clinic_id}}" @if(old('clinic_id') == $clinic_id) selected="" @endif>{{$clinic}}</option>
                        @endforeach
                      @endif
                    </select>
            ...
            <tr>
              ...
              <td>
                ...
                <input id="patient" class="input-text-mid form-control ui-autocomplete-input" name="patient" style="width: 250px; display: inline;" value="" autocomplete="off" type="text">
            ...
            <tr>
              ...
              <td>
                <select name="booking_recall_ym" id="booking_recall_ym" class="form-control form-control--xs" style="width: 90px !important;">
                      ...
                      <option value="{{dateAddMonth(date_now(), 01, 'Ym')}}" @if(old('booking_recall_ym') == dateAddMonth(date_now(), 01, 'Ym')) selected="" @endif >1ヶ月後</option>

                      <option value="{{dateAddMonth(date_now(), 02, 'Ym')}}" @if(old('booking_recall_ym') == dateAddMonth(date_now(), 02, 'Ym')) selected="" @endif >2ヶ月後</option>

                      <option value="{{dateAddMonth(date_now(), 03, 'Ym')}}" @if(old('booking_recall_ym') == dateAddMonth(date_now(), 03, 'Ym')) selected="" @endif >3ヶ月後</option>

                      <option value="{{dateAddMonth(date_now(), 04, 'Ym')}}" @if(old('booking_recall_ym') == dateAddMonth(date_now(), 04, 'Ym')) selected="" @endif >4ヶ月後</option>
                     
                      <option value="{{dateAddMonth(date_now(), 05, 'Ym')}}" @if(old('booking_recall_ym') == dateAddMonth(date_now(), 05, 'Ym')) selected="" @endif >5ヶ月後</option>

                      <option value="{{dateAddMonth(date_now(), 06, 'Ym')}}" @if(old('booking_recall_ym') == dateAddMonth(date_now(), 06, 'Ym')) selected="" @endif >6ヶ月後</option>
                    </select>

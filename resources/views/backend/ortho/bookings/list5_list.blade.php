        ...
        <table class="table table-bordered table-striped">
          <tbody>
            ...
            <tr>
              ...
              <td>{{ formatDate($list5->booking_date, '/') }} {{splitHourMin($list5->booking_start_time)}}～{{toTime($list5->booking_start_time, $list5->booking_total_time)}}</td>
              <td>{{ $list5->p_no }}</td>
              <td>{{ $list5->p_name }}</td>
              <td>{{ $list5->p_tel }}</td>
              <td>
                ...
                @if ( $list5->service_2_kind == 1 )
                  {{ @$services[$list5->service_2] }}
                @elseif ( $list5->service_2_kind == 2 )
                  {{ @$treatment1s[$list5->service_2] }}
                @endif
              </td>
